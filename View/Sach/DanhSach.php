				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					<div class="panel panel-default panel-sach">
						<div class="panel-body">
							...
							<div class="col-sm-8">
								<div class="tensach"><a href="chitietsanpham.php?madms={$row['MADMS']}&maloai={$row['MALOAI']}&masach={$row['MASACH']}">{$row['TENSACH']}</a></div>
				...
				<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
					<div class="panel panel-default panel-sach">
						<div class="panel-body text-center">
							<img class="biasach" src="hinh/{$row['HINH']}">
							$moi
							<div class="tensach text-ellipsis"><a href="chitietsanpham.php?madms={$row['MADMS']}&maloai={$row['MALOAI']}&masach={$row['MASACH']}">{$row['TENSACH']}</a></div>
							<div class="giaban text-danger">Giá bán: $gia VNĐ</div>
							<form method="post">
								<input type="hidden" name="masach" value="{$row['MASACH']}">
								<input type="hidden" name="btnThem">
...
EOD;
		...
		if(isset($count_book) && $count_book > 0 && $count_book > ITEMS_PAGE)
		{
			...
			else
			{
				...
				echo "<li><a href='trangchu.php$querystr&offset=$i'>&laquo;</a></li>";
			}

			for ($i=1; $i <= $totalpage; $i++) { 
				$value = ($i - 1) * ITEMS_PAGE;
				if($i == $curpage)
					echo "<li class='active'><a href='trangchu.php$querystr&offset=$value'>$i</a></li>";
				else
					echo "<li><a href='trangchu.php$querystr&offset=$value'>$i</a></li>";
			}
			...
			else
			{
				...
				echo "<li><a href='trangchu.php$querystr&offset=$i'>&raquo;</a></li>";
			}
